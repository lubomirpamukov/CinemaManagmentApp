...
const Schedule: React.FC<ScheduleProps> = ({
  ...
  const { sessions, loading, error, refetch, pagination, setPage } =
  ...
  if (error) {
    return (
      <div>
        Error: {error}{" "}
        <ActionButton id="retry" label="Retry" onClick={refetch} />
      </div>
    );
  }
  ...
  if (
    !loading &&
    sessions.length === 0 &&
    (cinemaId || hallId || movieId || date)
  ) {
    return (
      <div>
        No sessions found for the selected filters.{" "}
        <ActionButton id="refresh" label="Refresh" onClick={refetch} />
      </div>
    );
  }
  ...
  if (
    ...
    return (
      <div>
        No sessions to display. Try selecting some filters or{" "}
        <ActionButton id="refresh-all" label="Refresh All" onClick={refetch} />
      </div>
    );
  }
